...
export async function getFundamentalsAndNews(ticker) {
  ...
  try {
    ...
    // ---- News ----
    // Yahoo's search-based news lookup falls back to loosely-related or
    // generic trending articles when it can't find a good match for the
    // ticker (common for SGX-listed symbols) — only keep articles Yahoo
    // itself tags as being about this ticker, rather than show unrelated
    // headlines under this company's name.
    result.news = (searchRes.news || [])
